...
def lensing_f(thr_ct, smoothing, nside, itr, b, f):
    ...
    for i in range(itr):
        clustering_maps, lensing_maps = simulate_des_maps(0.3, 0.8, smoothing, nside, nmax=1)
        v_fixed, v0_fixed, v1_fixed, v2_fixed = calc_mf_f(lensing_maps[0], thr_ct, f, is_clustering=False)
        v_all_fixed[i] = np.concatenate((v0_fixed,v1_fixed,v2_fixed))
        print(i)
    ...
    for i in range(b):
        ...
        v,v0,v1,v2 = calc_mf_f(l_map[0], thr_ct, f, is_clustering=False)
    ...
    # save data in new subfolder
    sub_path = os.path.join(output_path, f't{thr_ct}_n{nside}_s{smoothing}_f{f}')
