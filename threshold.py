...
def threshold_mf_1(l_map_fixed,l_map,S_8,thr_ct,itr,nside,smoothing,map_len=1,b=10):
    ...
    v_all_fixed = np.zeros((itr,array_len)) 
    for i in range(itr):
        v_fixed, v0_fixed, v1_fixed, v2_fixed = calc_mf(l_map_fixed[i], thr_ct, N, is_clustering=False) 
        #calc_mf_2maps(c_map_fixed[i],l_map_fixed[i],thr_ct,N)
        v_all_fixed[i] = np.concatenate((v0_fixed.flatten(),v1_fixed.flatten(),v2_fixed.flatten()))
        print("Computing functionals for iteration", i)
    ...
    V_all = np.zeros((b,array_len))
    for i in range(b):
        v, v0, v1, v2 = calc_mf(l_map[i], thr_ct, N, is_clustering=False)
        #calc_mf_2maps(c_map[i],l_map[i],thr_ct,N)
        V_all[i] = np.concatenate((v0.flatten(),v1.flatten(),v2.flatten()))
        print(i)
...
def threshold_all(itr,thr_ct,map_len,nside=256,smoothing=20,b=10):
    ...
    print('Iterations - and so it begins...')
    for i in range(itr):
        c_map_fixed[i], l_map_fixed[i] = simulate_des_maps(0.3, 0.8, smoothing, nside, nmax=1)
        v_fixed, v0_fixed, v1_fixed, v2_fixed = calc_mf_2maps(c_map_fixed[i],l_map_fixed[i],thr_ct,N)
        #calc_mf(l_map_fixed[i], thr_ct, N, is_clustering=False) 
        v_all_fixed[i] = np.concatenate((v0_fixed.flatten(),v1_fixed.flatten(),v2_fixed.flatten()))
        print(i)
    ...
    for i in range(b):
        c_map[i], l_map[i] = simulate_des_maps(omega_m[i], sigma_8[i], smoothing, nside, nmax=1)
        v, v0, v1, v2 = calc_mf_2maps(c_map[i],l_map[i],thr_ct,N)
        #calc_mf(l_map[i], thr_ct, N, is_clustering=False)
        V_all[i] = np.concatenate((v0.flatten(),v1.flatten(),v2.flatten()))
        print(i)
